...
from model_pytorch import DoubleHeadModel, OpenAIModel
from text_utils import TextEncoder
from utils import (encode_dataset, iter_data,
                   ResultLogger, make_path)
from loss import ClassificationLossCompute, MultipleChoiceLossCompute
from parse_output import Prediction


class TrainingEngine():

    def transform_veracity(self, X1):
        n_batch = len(X1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vector_l2', action='store_true', default=True)
    ...
    parser.add_argument('--test_path', type=str, default='./data/RumorsAndNonRumors-TestingData.txt')
    parser.add_argument('--pred_path', type=str, default='./submission/veracity_states.tsv')
    parser.add_argument('--out_path', type=str, default='./results/predicted_results.txt')
    ...
    logger = ResultLogger(path=os.path.join(log_dir, '{}log.json'.format(desc)), **args.__dict__)
    ...
    ((trX, trY), (vaX, vaY), (teX,)) = encode_dataset(*dataLoader.veracity(data_dir, topic=topic),
    ...
    openAIModel.load_openai_pretrained_model(dh_model.transformer, n_ctx=n_ctx, n_special=n_special)
