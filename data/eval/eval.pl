...
my @cats = ("RUMOR", "NON-RUMOR", "UNKNOWN");
...
for (my $i = 0; $i < @guessLns; $i+=1) {
	...
	if($goldLn eq "ID	Target	Tweet	Veracity"){
		next;
	}
	...
	if(@goldArr != 4){
		...
		print STDERR "Correct format: ID<Tab>Target<Tab>Tweet<Tab>Veracity\n";
		die "\n";
	}
...
my $nCat = 0;
foreach my $cat (@cats) {
	if($cat eq "RUMOR" || $cat eq "NON-RUMOR"){
		$nCat += 1;
		$macroF += $fByCat{$cat};
